...
@interface BLTabPagerController ()<UICollectionViewDelegateFlowLayout, UICollectionViewDataSource>
{
    struct {
...
// views
@property (nonatomic, weak) UIView *superBarView;
...
@implementation BLTabPagerController

- (instancetype)initWithSuperBarView:(UIView *)superBarView {
    if (self = [self init]) {
        _superBarView = superBarView;
    }
...
- (void)configireTabPropertys {
    ...
    _barStyle = BLPagerBarStyleProgressView;
...
//    [_pagerBarView addSubview:collectionView];
    [_superBarView addSubview:collectionView];
...
- (void)addUnderLineView {
    ...
    underLineView.hidden = (_barStyle == BLPagerBarStyleNoneView);
    if (_barStyle != BLPagerBarStyleCoverView) {
...
- (void)setBarStyle:(BLPagerBarStyle)barStyle {
    _barStyle = barStyle;
    _progressView.hidden = (_barStyle == BLPagerBarStyleNoneView);
}

- (void)setDelegate:(id<BLTabPagerControllerDelegate>)delegate {
...
- (void)setDataSource:(id<BLPagerControllerDataSource>)dataSource {
...
- (void)layoutTabPagerView {
    ...
    CGFloat collectionViewEaging = _barStyle != BLPagerBarStyleCoverView ? _collectionLayoutEdging : (_collectionLayoutEdging > 0 ? _collectionLayoutEdging : -_progressEdging+_cellSpacing);
...
//    _collectionViewBar.frame = CGRectMake(0, [self statusBarHeight], CGRectGetWidth(self.view.frame), self.contentTopEdging);
    _collectionViewBar.frame = _superBarView.bounds;
}
...
- (void)setUnderLineFrameWithIndex:(NSInteger)index animated:(BOOL)animated {
    ...
    CGFloat progressY = _barStyle == BLPagerBarStyleCoverView ? (cellFrame.size.height - _progressHeight)/2:(cellFrame.size.height - _progressHeight);
...
- (void)setUnderLineFrameWithfromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex progress:(CGFloat)progress
{
    ...
    CGFloat progressY = _barStyle == BLPagerBarStyleCoverView ? (toCellFrame.size.height - _progressHeight)/2:(toCellFrame.size.height - _progressHeight);
    CGFloat progressX, width;
    
    if (_barStyle == BLPagerBarStyleProgressView) {
