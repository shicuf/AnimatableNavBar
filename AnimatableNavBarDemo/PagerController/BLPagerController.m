...
typedef NS_ENUM(NSUInteger, BLPagerControllerDirection) {
    BLPagerControllerLeft,
    BLPagerControllerRight,
    BLPagerControllerNone,
};

@interface BLPagerController ()<UIScrollViewDelegate> {
...
@implementation BLPagerController
...
- (void)setDelegate:(id<BLPagerControllerDelegate>)delegate {
    _delegate = delegate;
...
- (void)configurePagerIndex {
    ...
    // scroll direction
    BLPagerControllerDirection direction = offsetX >= _preOffsetX ? BLPagerControllerLeft : BLPagerControllerRight;
    ...
    // caculate cur index
    if (direction == BLPagerControllerLeft) {
