...
@implementation BLAnimatableNavBar
...
- (void)layoutSubviews {
    ...
    if(self.behavior && self.behavior.isElastic)
    {
        ...
        for (int i = 0; i < [subview numberOfLayoutAttributes]; i++) {
            ...
            if (i+1 < [subview numberOfLayoutAttributes]) {
                ...
                if (self.progress >= floorProgressPosition
                    && self.progress < ceilingProgressPosition) {
                    BLSubviewLayoutAttributes *floorLayoutAttributes = [subview layoutAttributesAtIndex:i];
                    BLSubviewLayoutAttributes *ceilingLayoutAttributes = [subview layoutAttributesAtIndex:i+1];
...
- (void)applyFloorLayoutAttributes:(BLSubviewLayoutAttributes *)floorLayoutAttributes ceilingLayoutAttributes:(BLSubviewLayoutAttributes *)ceilingLayoutAttributes toSubview:(UIView *)subview withFloorProgress:(CGFloat)floorProgress ceilingProgress:(CGFloat)ceilingProgress {
    ...
    // Interpolate frame
    CGRect frame;
    if (!CGRectEqualToRect(floorLayoutAttributes.frame, CGRectNull)
        && CGRectEqualToRect(ceilingLayoutAttributes.frame, CGRectNull)) {
        frame = floorLayoutAttributes.frame;
    } else if (CGRectEqualToRect(floorLayoutAttributes.frame, CGRectNull)
               && CGRectEqualToRect(ceilingLayoutAttributes.frame, CGRectNull)) {
        frame = subview.frame;
    } else {
    ...
    subview.alpha = alpha;
    subview.layer.zPosition = floorLayoutAttributes.originZ;
...
- (void)setProgress:(CGFloat)progress {
    ...
    if ((self.behavior
         && !self.behavior.isElastic)
        || !self.behavior) {
        _progress = fmax(_progress, 0.0);
    }
}

- (void)setBehavior:(BLAnimatableNavBarBehavior *)behavior {
    _behavior = behavior;
